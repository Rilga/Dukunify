            ...
            <div class="glass-dark rounded-2xl shadow-2xl shadow-purple-900/50 border border-purple-700/30 mystical-glow overflow-hidden">
                <div class="p-6 sm:p-8 text-purple-100">
                    ...
                    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        ...
                        <div class="mt-4">
                            <label for="password" class="block font-medium text-sm text-purple-300 mb-1">{{ __('Password (Opsional)') }}</label>
                            <input id="password" type="password" name="password"
                                   class="block mt-1 w-full bg-gray-900/50 border-purple-700/50 border focus:border-purple-500 focus:ring-purple-500 rounded-lg shadow-sm text-gray-100 placeholder-gray-500">
                            <x-input-error :messages="$errors->get('password')" class="mt-2" />
                            <p class="text-sm text-purple-400 mt-1">Kosongkan jika Anda tidak ingin mengubah password.</p>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="role" name="role" style="color-scheme: dark;"
                                ...
                                <option value="user" {{ old('role', $user->role) == 'user' ? 'selected' : '' }} class="bg-gray-900 text-purple-200">User (Klien)</option>
                                <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }} class="bg-gray-900 text-purple-200">Admin</option>
                            </select>
