            ...
            <div class="glass-dark rounded-2xl shadow-2xl shadow-purple-900/50 border border-purple-700/30 mystical-glow overflow-hidden">
                <div class="p-6 md:p-8 text-purple-100 space-y-8">
                    ...
                    <div class="text-center">
                     ...
                     {{-- Detail Klien --}}
                    <div>
                        <h3 class="text-xl font-bold text-purple-200 border-b border-purple-700/50 pb-3 mb-4">Detail Klien</h3>
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                            <dt class="font-medium text-purple-400">Nama Klien:</dt>
                            <dd class="font-semibold text-purple-200">{{ $booking->user->name }}</dd>
                            <dt class="font-medium text-purple-400">Email Klien:</dt>
                            <dd class="text-purple-200">{{ $booking->user->email }}</dd>
                             {{-- Menampilkan Nomor Telepon Klien --}}
                            <dt class="font-medium text-purple-400">Nomor Telepon:</dt>
                            <dd class="text-purple-200">{{ $booking->user->profile->phone_number ?? '-' }}</dd>
                            {{-- Menampilkan Alamat Klien --}}
                            <dt class="font-medium text-purple-400">Alamat:</dt>
                            <dd class="text-purple-200">{{ $booking->user->profile->address ?? '-' }}</dd>
                        </dl>
                    </div>
                    ...
                    <div>
                        ...
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                            ...
                            <dt class="font-medium text-purple-400">Tanggal Selesai Aktual (Dikonfirmasi):</dt>
                     ...
                     @if ($booking->status == 'menunggu_konfirmasi_pembayaran')
                        <div class="mt-6 pt-6 border-t border-purple-700/50 text-center">
                            <form action="{{ route('admin.approvals.confirmPayment', $booking) }}" method="POST" onsubmit="return confirm('Konfirmasi pembayaran ini dan selesaikan transaksi?');">
                                @csrf
                                {{-- Tombol Konfirmasi (Hijau Mystical) --}}
                                <button type="submit"
                                    ...
                                    <span class="relative z-10">{{ __('Konfirmasi Pembayaran & Selesaikan') }}</span>
                    ...
                    <div class="mt-6 text-center">
                        <a href="{{ route('admin.reports.history') }}" class="text-sm text-purple-400 hover:text-purple-200 transition duration-300">&larr; Kembali ke Riwayat Booking</a>
                    </div>
