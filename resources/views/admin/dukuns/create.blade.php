        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.dukuns.store') }}" method="POST" enctype="multipart/form-data"> {{-- <<< Tambahkan enctype --}}
                        @csrf
                        
                        <div>
                            <x-input-label for="kode_dukun" :value="__('Kode Dukun (Unik)')" />
                            <x-text-input id="kode_dukun" class="block mt-1 w-full" type="text" name="kode_dukun" :value="old('kode_dukun')" required autofocus />
                            <x-input-error :messages="$errors->get('kode_dukun')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="nama_dukun" :value="__('Nama Dukun')" />
                            <x-text-input id="nama_dukun" class="block mt-1 w-full" type="text" name="nama_dukun" :value="old('nama_dukun')" required />
                        ...
                        <div class="mt-4">
                            <x-input-label for="image" :value="__('Gambar Dukun')" />
